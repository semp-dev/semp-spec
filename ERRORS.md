...
| `server_unavailable`  | Yes         | The recipient server is transiently unreachable: discovery succeeded but no transport to it could be opened, or a `.onion` recipient's well-known fetch failed and clearnet fallback is forbidden (`DISCOVERY.md` §2.5.2), or a key fetch propagated a transient transport failure (`KEY.md` §6.4). Back off and retry per `DELIVERY.md` §2.3. Do NOT switch to a different recipient address. |
